...
namespace IdentityServer4.Fsql.Storage.Stores
{
    ...
    public class DeviceFlowStore : IDeviceFlowStore
    {
        ...
        {
            var deviceFlowCodes = await FreeSql.Select<Entities.DeviceFlowCodes>().Where(x => x.UserCode == userCode).FirstAsync();
        ...
        {
            var existing = await FreeSql.Select<Entities.DeviceFlowCodes>().Where(x => x.UserCode == userCode).FirstAsync();
            ...
            try
            {
                await FreeSql.Update<Entities.DeviceFlowCodes>().SetSource(existing).ExecuteAffrowsAsync();
            }
            catch (Exception ex)
            {
